...
void Server_IOB::broadcastClientUpdate(Client* cl)
{
	...
	{
		if (!(*cIter)->getWebsocket())
		{
			// todo update failed for certain client
		}
		else
		{
			(*cIter)->getWebsocket()->sendTextMessage(bcUpdate);
...
// send all known clients to new client
void Server_IOB::sendClientListToNewClient(Client* cl) 
{
	bool done = true;
	QList<Client*>::ConstIterator cIter = mClients.constBegin();
	...
	while (cIter != mClients.constEnd())
	{
		// create update telegram
		QString bcUpdate(QString::number(MESSAGEID::UPDATE));
		bcUpdate.append("#").append((*cIter)->getUuid().toString());
		bcUpdate.append("#").append((*cIter)->getName());
		bcUpdate.append("#").append(QString::number((*cIter)->getStatus()));
		bcUpdate.append("#").append((*cIter)->getLocation());
		bcUpdate.append("#").append((*cIter)->getPhone());
		bcUpdate.append("#").append((*cIter)->getNotes());
		bcUpdate.append("#").append(QDateTime::currentDateTime().toString());
		qDebug() << bcUpdate;
		if (!cl->getWebsocket()->sendTextMessage(bcUpdate))
		{
			// todo update failed for sending client data
			qDebug() << "Didnt sent client:" << (*cIter)->getUuid();
		}
		else
		{
			qDebug() << "Sent client:" << (*cIter)->getUuid();
		}
